...
- **Tracker**: registro de actividades sostenibles con **medición GPS real** (usa `navigator.geolocation.watchPosition` para medir la distancia efectivamente recorrida, con filtrado de ruido y velocidad/tiempo reales), más un modo manual alternativo honestamente etiquetado como tal (para casos como bicicleta estática o sin GPS disponible).
...
- **Comunidad (Feed Social)**: muestra actividades reales de todos los usuarios (vía Firestore), con likes reales persistentes y una estadística real de CO2 mitigado por la comunidad. Los posts de ejemplo originales se mantienen, claramente etiquetados como tal y sin interacción posible.
- **Rewards**: sistema de puntos ("Beatcoins", ganados en Track) y canje de recompensas. La entrega física se coordina manualmente por **WhatsApp** (`wa.me/56939010237`, mensaje prellenado con el código de canje), y el equipo administra los canjes pendientes desde el Panel de Admin → "Canjes Pendientes de Entrega". Incluye Términos y Condiciones de Canje visibles para el usuario. Las tarjetas de premios usan iconos genéricos uniformes (sin fotos de stock).
